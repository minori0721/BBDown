...
- `bfb-2.0.7`：精确大小探测沿用实际下载请求的 Referer 规则；APP 与安卓 TV 媒体地址不发送 Referer，避免部分 CDN 拒绝 HEAD/Range 探测。普通 WEB 媒体地址保持原有 Referer 行为。
- `bfb-2.0.6`：恢复 APP 请求的 TLS 证书校验；调试日志不再输出 Cookie、签名地址、播放响应及请求正文；收藏夹接口检查业务状态并释放分页响应；章节接口异常会提示并继续不带章节下载。
...
- 结构化探测支持集成方传入隐藏参数`--bfb-probe-quality`和`--bfb-probe-encoding`；仅对排序后首个匹配视频流及其音频流并发做一次HEAD，必要时回退到单字节Range读取精确总大小。`206`缺少完整`Content-Range`时不会把单字节响应误认成文件总大小；未指定严格目标时仍只返回接口大小或码率估算。
